...
Использование директивы `try_files` подразумевает что вы можете тестировать не одно условие а последовательность (цепочку) из нескольких условий. Например, в данном примере проверяется существует ли файл по указанному `$uri`, если файл не найден, то проверяется существование директории `$uri/`, если и ее нет, то проверка идет далее пока не сработает условие, конечное условие сработает по умолчанию (в данном примере это `/index.html`), если совпадение так и не найдтеся.
