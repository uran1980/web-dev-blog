...
На самом деле здесь несколько проблемных моментов... Первое это `if` (смотрим статью **[`if` - это Зло!!!](https://github.com/uran1980/web-deb-blog/blob/master/Nginx/if-is-evil.md)**). Когда nginx получает запрос, не важно в какой поддомен он направлен, будь это `www.domain.com` или `domain.com`, условие `if` **всегда сработает!!!** В примере получается, что мы указываем, чтобы nginx проверял заголовок хоста ([встроенную переменную](http://nginx.org/ru/docs/http/ngx_http_core_module.html#variables) `$host`) **при каждом запросе**. Это крайне не эффективно. Вы должны избегать этого. Вместо неудачного условия `if` лучше использовать два блока с директивой [`server`](http://nginx.org/ru/docs/http/ngx_http_core_module.html#server), как в примере ниже.
