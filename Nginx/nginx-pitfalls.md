...
В итоге мы получили читабельный конфиг. Так же, мы уменьшили затраты ресурсов nginx на обработку запросов. A еще мы избавились от условия `if` и использовали свтроенную переменную `$scheme`, которая позволяет не привязывать **[URI](http://ru.wikipedia.org/wiki/URI)** к определенному протоколу, будь то `http` или `https`.
...
Если вы явно не задали директиву [`underscores_in_headers`](http://nginx.org/ru/docs/http/ngx_http_core_module.html#underscores_in_headers) в значение `on`, то nginx по-умолчанию будет тихо и незаметно пропускать HTTP заголовки, содержащие символы нижнего подчеркивания (которые на самом деле, с точки зрения описания протокола HTTP, вполне допустимы). Nginx делает это только для исключения неоднозначности при маппинге заголовков в переменные при передаче в CGI скрипты, и преобразует символы `-` и `_` в символы нижнего подчеркивания `_`.
