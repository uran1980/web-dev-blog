...
Если вы явно не задали директиву в значение [`underscores_in_headers`](http://nginx.org/ru/docs/http/ngx_http_core_module.html#underscores_in_headers) в значение `on`, то nginx по-умолчанию будет тихо и незаметно пропускать HTTP заголовки, содержащие симолы нижнего подчеркиваеия (которые на самом деле с точки зрения описания протокола HTTP вполне допустимы). Nginx делает это только для исключения неоднознаности при маппинге заголовков в переменные при передаче в CGI скрипты, и преобразует символы `-` и `_` в символы нижнего подчеркивания `_`.
