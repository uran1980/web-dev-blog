...
Т.к. ssh-клиент по-умолчанию не распространяется на Windows платформе, то при запуске команды `vagrant shh` на Windows машине **Vagrant** выведет вместо окна терминала окно с необходимой информацией для подключения к виртуальной машине по shh протоколу
***
