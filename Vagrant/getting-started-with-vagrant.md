...
Мы запустили с помощью **Vagrant** виртуальную машину, настроили подключение через **ssh**, и что дальше? Для нас как для разработчиков эта машина не представляет никакого инетерса, т.к. на ней ничего не установлено кроме оперционной системы **Ubuntu**. Для разработки нашего web-проекта нам необходимо из получившейся заготовки виртуальной машины сделать полноценное виртуальное окружение - максимально точную копию того, что у нас находится на рабочем сервере. Для этого нам надо установить нужное программное обеспечение, к примеру, **nginx** или **Apache**, **php**, **ruby**, **MySQL** и т.д., т.е обеспечить наш виртуальный бокс необходимой **провизией (Provisioning)**

Для этих целей **Vagrant** позволяет использовать на выбор одно из двух решений для автоматизированного развертывания необходимых приложений на виртуальной машине - это **[Chef (Chef-Solo)](http://docs.vagrantup.com/v1/docs/provisioners/chef_solo.html)** или **[Puppet](http://docs.vagrantup.com/v1/docs/provisioners/puppet.html)**, таже при необходимости вы можете расширять **Vagrant** и использовать **[другие средства](http://docs.vagrantup.com/v1/docs/provisioners/puppet.html)** автоматизации развертывания приложение (но это уже не относится к теме данной статьи).
