...
Итак мы получили готовое для использования виртуальное окружение для разработки web-проекта. Но представьте ситуацию, что вы не один, а состоите в команде разрабочиков и вам надо обеспечить своиг коллег такимже окружением для разработки проекта. Что делать? **Vagrant** предоставляет для этого случая возможность создавать **пакеты (packages)** виртуального окружения. Эти готовые пакеты вы отдаете своим коллегам и они устанавливают их у себя на компьютере.

**Пакеты (Packages)** это готовые к экспорту образы вашего виртуального окружения, которое могут быть быстро развернуты на других компьютерах. Обычно они имеют расширение `.box` и называются боксами. Опционально в этот бокс вы можете также включить сам конфигурационный файл `Vagrantfile`.
...
Сначала мы должны создать конфигурационный файл `Vagrantfile`, который затем будет помещен в созданный виртуальный бокс.  На этот этапе важно убетиться, что ваше виртуальное окружение запущено командой `vagrant up`, работает HTTP доступ к нему. Теперь создаем файл `Vagrantfile.pkg`, который будет использоваться как `Vagrantfile` для бокса. Файл должен содержать следующую конфигурацию:
```ruby
Vagrant::Config.run do |config|
  # Forward apache
